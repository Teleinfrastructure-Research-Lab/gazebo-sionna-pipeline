...
The repository also includes an experiment-local extension documented in
[`docs/semantic_ablation_200f_pipeline.md`](./semantic_ablation_200f_pipeline.md).
That branch reuses the same validated core inputs and environments listed here,
then produces additional generated RT/feature/result artifacts under
`rt_out/experiments/`.
...
Experiment-local semantic ablation branches generate analogous files under
`rt_out/experiments/<experiment_name>/...`, plus RT CSVs, labeled CSVs, and
feature tables. Those are useful working artifacts, but they are usually large
generated outputs and should not be committed casually.
...
- **Generated by experiment-local wrappers**
  - sampled-frame JSON
  - experiment-local dynamic mesh / manifest / XML indexes
  - multi-RX RT CSVs
  - RT-derived labels
  - object-aware feature tables
  - raw occupancy baseline tables
  - classical-model result CSVs
...
- protection against accidentally committing large generated experiment outputs
