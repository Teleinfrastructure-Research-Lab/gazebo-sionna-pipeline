...
DEFAULT_ACTOR_SAMPLES = PROJECT_ROOT / "rt_out" / "experiments" / "factory_panda_ur5" / "legacy_run_20260522_133045" / "dynamic_frames" / "actor_frame_samples.json"
...
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "rt_out" / "experiments" / "factory_panda_ur5" / "legacy_run_20260522_133045" / "dynamic_scene"
...
def validate_component_summary(export: dict[str, Any]) -> list[dict[str, Any]] | None:
    """Accept old worker summaries, but strictly validate new stable-index metadata."""
    raw_components = export.get("components")
    if raw_components is None:
        return None
    if not isinstance(raw_components, list) or not raw_components:
        raise ActorFrameExportError(f"{export['id']}.components must be a non-empty list")
    names: list[str] = []
    expected_offset = 0
    result: list[dict[str, Any]] = []
    for index, raw in enumerate(raw_components):
        if not isinstance(raw, dict):
            raise ActorFrameExportError(f"{export['id']}.components[{index}] must be an object")
        name = require_non_empty_string(raw.get("name"), f"{export['id']}.components[{index}].name")
        offset = int(raw.get("vertex_offset", -1)); count = int(raw.get("vertex_count", 0))
        triangles = int(raw.get("triangle_count", 0))
        topology_hash = require_non_empty_string(raw.get("topology_hash"), f"{export['id']}.components[{index}].topology_hash")
        if offset != expected_offset or count <= 0 or triangles <= 0:
            raise ActorFrameExportError(f"{export['id']}.components[{index}] has invalid offset/counts")
        expected_offset += count; names.append(name)
        result.append({"name": name, "vertex_offset": offset, "vertex_count": count, "triangle_count": triangles, "topology_hash": topology_hash})
    if names != sorted(names) or len(names) != len(set(names)):
        raise ActorFrameExportError(f"{export['id']} component names are duplicate or not deterministically sorted")
    if expected_offset != int(export.get("mesh_vertex_count", -1)):
        raise ActorFrameExportError(f"{export['id']} component offsets do not cover mesh_vertex_count")
    return result
...
def build_output_manifest(
    ...
    for sample in worker_samples:
        ...
        components = validate_component_summary(export)
...
def main() -> int:
    ...
    # The worker consumes a temporary spec; existing metadata remains valid until
    # Blender succeeds and the final PLY/summary/manifest replacements complete.
    temporary_spec_path = config.worker_spec_path.with_name(config.worker_spec_path.name + ".tmp")
    save_json(temporary_spec_path, spec)

    blender = resolve_blender_or_raise(args.blender)
    run_blender(blender, temporary_spec_path)
    ...
    save_json_atomic(config.output_manifest_path, output_manifest)
    os.replace(temporary_spec_path, config.worker_spec_path)
