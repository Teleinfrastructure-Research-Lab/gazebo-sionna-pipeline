...
OUTPUT_PATH = PROJECT_ROOT / "rt_out" / "experiments" / "factory_panda_ur5" / "legacy_run_20260522_133045" / "dynamic_frames" / "prototype_frames.json"
...
def load_selected_frames(
    path: Path | None,
    *,
    default_frames: list[dict[str, int]],
) -> tuple[list[dict[str, int]], Path | None]:
    ...
    prototype_config_path: Path | None = None
    if isinstance(raw, dict):
        raw_frames = raw.get("frames")
        prototype_config_value = raw.get("dynamic_prototype_config_path")
        if prototype_config_value not in (None, ""):
            if not isinstance(prototype_config_value, str) or not prototype_config_value.strip():
                raise DynamicFrameError(
                    "dynamic_prototype_config_path must be a non-empty string when present"
                )
            prototype_config_path = Path(prototype_config_value).expanduser().resolve()
    else:
        raw_frames = raw
    ...
    return frames, prototype_config_path
...
def load_dynamic_models(
    ...
    manifest_path: Path = DYNAMIC_MANIFEST_PATH,
    ...
    for entry in manifest:
        ...
        model_config = prototype_config["dynamic_models"][model_name]
        expected_count = model_config["expected_link_count"]
...
def main() -> int:
    ...
    manifest_path = args.dynamic_manifest or (
        experiment_root / "manifests" / "dynamic_manifest.json" if experiment_root else DYNAMIC_MANIFEST_PATH
    )
    output_path_arg = args.output or (
        experiment_root / "dynamic_frames" / "prototype_frames.json" if experiment_root else OUTPUT_PATH
    )
    prototype_config = load_dynamic_prototype_config(config_path) if config_path else load_dynamic_prototype_config()
    frame_records, frames_prototype_config_path = load_selected_frames(
        args.frames_json,
        default_frames=prototype_config["prototype_frames"],
    )
    if frames_prototype_config_path is not None:
        prototype_config = load_dynamic_prototype_config(frames_prototype_config_path)
    ...
    output_path = output_path_arg.expanduser().resolve()
    dynamic_models = load_dynamic_models(prototype_config, manifest_path.expanduser().resolve())
    ...
    output = {
        ...
        "dynamic_pose_logs": {
            model_name: str(dynamic_models[model_name]["log_path"])
            for model_name in model_order
        },
