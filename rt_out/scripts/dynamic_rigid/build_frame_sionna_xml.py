...
class XmlConfig:
    ...
    rt_material_config_path: Path | None
...
def configure_prototype_config(args: argparse.Namespace) -> None:
    global PROTOTYPE_CONFIG, DYNAMIC_MODEL_MATERIALS, ALLOWED_DYNAMIC_MODELS
...
def build_config(args: argparse.Namespace) -> XmlConfig:
    ...
    return XmlConfig(
        ...
        rt_material_config_path=(
            args.rt_material_config.expanduser().resolve()
            if args.rt_material_config else None
        ),
    )
...
def resolve_entry_material(entry: dict[str, Any], warnings: list[str]) -> str:
    ...
    # Actor meshes are baked world geometry from their own branch and keep their
    # explicit material label, e.g. human_skin.
    ...
    if source == "actor":
        return require_non_empty_string(entry.get("material_label"), f"Actor entry {entry_id}.material_label")
...
def validate_and_resolve_entries(
    ...
    *,
    ...
    if not isinstance(actor_count, int):
        raise FrameSionnaXmlError(f"actor_count must be an integer, got {actor_count!r}")
    if static_count + dynamic_count + actor_count != total_count:
        raise FrameSionnaXmlError(
            f"static_count + dynamic_count + actor_count = {static_count + dynamic_count + actor_count}, "
    ...
    if source_counts.get("actor", 0) != actor_count:
        raise FrameSionnaXmlError(
            f"Manifest actor_count={actor_count}, but resolved actor entries="
            f"{source_counts.get('actor', 0)}"
        )
...
def main() -> int:
    ...
    try:
        ...
        material_specs = load_rt_material_specs(config.rt_material_config_path)
    ...
    print(f"Actor entries: {source_counts.get('actor', 0)}")
