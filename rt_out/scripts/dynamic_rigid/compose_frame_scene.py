...
STATIC_MANIFEST_PATH = PROJECT_ROOT / "rt_out" / "experiments" / "factory_panda_ur5" / "legacy_run_20260522_133045" / "static_scene" / "export" / "merged_static_manifest.json"
DEFAULT_DYNAMIC_SCENE_ROOT = PROJECT_ROOT / "rt_out" / "experiments" / "factory_panda_ur5" / "legacy_run_20260522_133045" / "dynamic_scene"
DEFAULT_COMPOSED_SCENE_ROOT = PROJECT_ROOT / "rt_out" / "experiments" / "factory_panda_ur5" / "legacy_run_20260522_133045" / "composed_scene"

PROTOTYPE_CONFIG: dict[str, Any] = {}
EXPECTED_DYNAMIC_COUNT = PROTOTYPE_CONFIG.get("expected_renderable_visual_count_total", 0)
...
class ComposeConfig:
    ...
    actor_frame_manifest_path: Path | None
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional actor_frame_XXX_manifest.json path to append baked actor meshes.",
    )
    return parser.parse_args()


def configure_prototype_config(args: argparse.Namespace) -> None:
    global PROTOTYPE_CONFIG, EXPECTED_DYNAMIC_COUNT
    PROTOTYPE_CONFIG = load_dynamic_prototype_config(
        args.dynamic_prototype_config.expanduser().resolve()
        if args.dynamic_prototype_config
        else None
    )
    EXPECTED_DYNAMIC_COUNT = PROTOTYPE_CONFIG.get("expected_renderable_visual_count_total", 0)
...
def build_config(args: argparse.Namespace) -> ComposeConfig:
    ...
    return ComposeConfig(
        ...
        actor_frame_manifest_path=(
            args.actor_frame_manifest.expanduser().resolve()
            if args.actor_frame_manifest is not None
            else None
        ),
...
def build_actor_entries(
    actor_frame_manifest: dict[str, Any],
    *,
    frame_id: int,
    source_sample_index: int,
) -> list[dict[str, Any]]:
    # Actor meshes are exported as already-baked world geometry in a separate
    # branch from the Panda/UR5 rigid visual path. They are appended only when
    # the caller explicitly passes an actor frame manifest.
    if actor_frame_manifest.get("frame_id") != frame_id:
        raise ComposeFrameSceneError(
            f"Actor manifest frame_id={actor_frame_manifest.get('frame_id')}, expected {frame_id}"
        )
    actor_manifest_source_sample_index = actor_frame_manifest.get("source_sample_index")
    if (
        actor_manifest_source_sample_index is not None
        and actor_manifest_source_sample_index != source_sample_index
    ):
        raise ComposeFrameSceneError(
            "Actor manifest source_sample_index="
            f"{actor_manifest_source_sample_index}, expected dynamic source_sample_index={source_sample_index}"
        )

    actors = actor_frame_manifest.get("exported_actors")
    if not isinstance(actors, list):
        raise ComposeFrameSceneError("Actor frame manifest must contain exported_actors list")

    entries: list[dict[str, Any]] = []
    for index, actor in enumerate(actors):
        if not isinstance(actor, dict):
            raise ComposeFrameSceneError(f"Actor exported_actors[{index}] is not an object")
        actor_id = actor.get("id")
        if not isinstance(actor_id, str) or not actor_id:
            raise ComposeFrameSceneError(f"Actor exported_actors[{index}].id must be a non-empty string")
        if actor.get("source") != "actor":
            raise ComposeFrameSceneError(f"Actor {actor_id} must have source == 'actor'")
        if actor.get("baked_world_geometry") is not True:
            raise ComposeFrameSceneError(f"Actor {actor_id} must have baked_world_geometry == true")
        if actor.get("source_sample_index") != source_sample_index:
            raise ComposeFrameSceneError(
                f"Actor {actor_id} has source_sample_index={actor.get('source_sample_index')}, "
                f"expected dynamic source_sample_index={source_sample_index}"
            )
        material_label = actor.get("material_label")
        if not isinstance(material_label, str) or not material_label.strip():
            raise ComposeFrameSceneError(f"Actor {actor_id} must have a non-empty material_label")

        mesh_path, rerooted = resolve_existing_path(
            actor.get("exported_mesh_path"), f"actor {actor_id}.exported_mesh_path"
        )
        if rerooted:
            raise ComposeFrameSceneError(f"Actor {actor_id} unexpectedly required path rerooting")
        ...
        entries.append(
            {
                ...
                "frame_id": actor.get("frame_id"),
                "source_sample_index": actor.get("source_sample_index"),
                "baked_world_geometry": True,
                "alignment_policy": actor.get("alignment_policy"),
                "z_alignment_policy": actor.get("z_alignment_policy"),
                "floor_z": actor.get("floor_z"),
                "mesh_vertex_count": actor.get("mesh_vertex_count"),
                "mesh_face_count": actor.get("mesh_face_count"),
                "bounds_min": actor.get("bounds_min"),
                "bounds_max": actor.get("bounds_max"),
                "actor_time_seconds": actor.get("actor_time_seconds"),
                "animation_time_seconds": actor.get("animation_time_seconds"),
                "root_pose_source": actor.get("root_pose_source"),
                "root_pose6": actor.get("root_pose6"),
                "skin_scale": actor.get("skin_scale"),
                "actor_record": actor,
            }
        )
...
def validate_entries(
    ...
    if static_count + dynamic_count + actor_count != len(entries):
...
def build_manifest(config: ComposeConfig) -> dict[str, Any]:
    ...
    if config.actor_frame_manifest_path is not None:
        actor_manifest = load_json(config.actor_frame_manifest_path)
        if not isinstance(actor_manifest, dict):
            raise ComposeFrameSceneError("Actor frame manifest root must be an object")
        actor_entries = build_actor_entries(
            actor_manifest,
            frame_id=config.frame_id,
            source_sample_index=source_sample_index,
        )
    ...
    # dynamic entries and optional baked actor entries for this frame. No
    # geometry is modified here.
    entries = static_entries + dynamic_entries + actor_entries

    static_count = len(static_entries)
    dynamic_count = len(dynamic_entries)
    actor_count = len(actor_entries)
    validate_entries(entries, static_count, dynamic_count, actor_count)
    ...
    return {
        ...
        "actor_source_manifest": (
            relpath(config.actor_frame_manifest_path)
            if config.actor_frame_manifest_path is not None
            else None
        ),
        "static_entry_kind": "merged_material_group",
        ...
        "total_count": static_count + dynamic_count + actor_count,
        "entries": entries,
        "validation": {
            "frame_id": config.frame_id,
            "source_sample_index": source_sample_index,
...
def main() -> int:
    args = parse_args()
    configure_prototype_config(args)
    ...
    print(f"actor_count: {manifest['actor_count']}")
