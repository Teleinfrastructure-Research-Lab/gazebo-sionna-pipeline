...
DYNAMIC_VISUAL_FRAMES_PATH = PROJECT_ROOT / "rt_out" / "experiments" / "factory_panda_ur5" / "legacy_run_20260522_133045" / "dynamic_frames" / "dynamic_visual_frames.json"
DEFAULT_DYNAMIC_OUTPUT_ROOT = PROJECT_ROOT / "rt_out" / "experiments" / "factory_panda_ur5" / "legacy_run_20260522_133045" / "dynamic_scene"
...
EXPECTED_SOURCE_SAMPLE_BY_FRAME = PROTOTYPE_CONFIG.get("source_sample_by_frame", {})
EXPECTED_RENDERABLE_VISUALS = PROTOTYPE_CONFIG.get("expected_renderable_visual_count_total", 0)
...
def configure_prototype_config(args: argparse.Namespace) -> None:
    global PROTOTYPE_CONFIG, MODEL_ORDER, EXPECTED_SOURCE_SAMPLE_BY_FRAME
    global EXPECTED_RENDERABLE_VISUALS, EXPECTED_PER_MODEL_COUNTS
...
def blender_executable() -> Path:
    # The dynamic export path relies on Blender for every frame, so centralize
    # the lookup logic with the other exporters.
    try:
        return find_blender()
    except RuntimeConfigError as exc:
        raise DynamicGeometryExportError(str(exc)) from exc
...
def run_blender_worker(config: ExportConfig) -> int:
    ...
    manifest = {
        ...
        "source_visual_frames_file": (
            "rt_out/experiments/factory_panda_ur5/legacy_run_20260522_133045/dynamic_frames/dynamic_visual_frames.json"
            if config.visual_frames_path == DYNAMIC_VISUAL_FRAMES_PATH
