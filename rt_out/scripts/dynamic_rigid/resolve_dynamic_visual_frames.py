...
FRAMES_PATH = PROJECT_ROOT / "rt_out" / "experiments" / "factory_panda_ur5" / "legacy_run_20260522_133045" / "dynamic_frames" / "prototype_frames.json"
DYNAMIC_MANIFEST_PATH = PROJECT_ROOT / "rt_out" / "experiments" / "factory_panda_ur5" / "legacy_run_20260522_133045" / "manifests" / "dynamic_manifest.json"
OUTPUT_PATH = PROJECT_ROOT / "rt_out" / "experiments" / "factory_panda_ur5" / "legacy_run_20260522_133045" / "dynamic_frames" / "dynamic_visual_frames.json"

PROTOTYPE_CONFIG: dict[str, Any] = {}
MODEL_ORDER = PROTOTYPE_CONFIG.get("model_names", [])
EXPECTED_SAMPLE_INDICES = PROTOTYPE_CONFIG.get("source_sample_indices", [])
EXPECTED_FRAME_IDS = PROTOTYPE_CONFIG.get("frame_ids", [])
EXPECTED_COUNTS = {
    model_name: {
    ...
    for model_name, model_config in PROTOTYPE_CONFIG.get("dynamic_models", {}).items()
}
EXPECTED_TOTAL_RENDERABLE_VISUALS = PROTOTYPE_CONFIG.get("expected_renderable_visual_count_total", 0)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional run root; derives config/, manifests/, and dynamic_frames/ paths.",
    )
    parser.add_argument(
        "--dynamic-prototype-config",
        type=Path,
        default=None,
        help="Dynamic prototype configuration JSON. Overrides --experiment-root/config/.",
    )
    parser.add_argument(
        "--models-root",
        type=Path,
        default=None,
        help="Root containing dynamic model assets. Defaults to the repository models/ directory.",
    )
    parser.add_argument(
        "--dynamic-manifest",
        type=Path,
        default=None,
        help="Extracted dynamic manifest JSON. Overrides --experiment-root/manifests/.",
    )
    parser.add_argument(
        "--frames-json",
        type=Path,
...
def configure_paths(args: argparse.Namespace) -> None:
    global DYNAMIC_MANIFEST_PATH, FRAMES_PATH, MODELS_ROOT, OUTPUT_PATH
    global PROTOTYPE_CONFIG, MODEL_ORDER, EXPECTED_SAMPLE_INDICES, EXPECTED_FRAME_IDS
    global EXPECTED_COUNTS, EXPECTED_TOTAL_RENDERABLE_VISUALS
    experiment_root = args.experiment_root.expanduser().resolve() if args.experiment_root else None
    config_path = args.dynamic_prototype_config or (
        experiment_root / "config" / "dynamic_prototype_config.json" if experiment_root else None
    )
    DYNAMIC_MANIFEST_PATH = (args.dynamic_manifest or (
        experiment_root / "manifests" / "dynamic_manifest.json" if experiment_root else DYNAMIC_MANIFEST_PATH
    )).expanduser().resolve()
    FRAMES_PATH = (args.dynamic_frames or (
        experiment_root / "dynamic_frames" / "prototype_frames.json" if experiment_root else FRAMES_PATH
    )).expanduser().resolve()
    OUTPUT_PATH = (args.output or (
        experiment_root / "dynamic_frames" / "dynamic_visual_frames.json" if experiment_root else OUTPUT_PATH
    )).expanduser().resolve()
    MODELS_ROOT = (args.models_root or MODELS_ROOT).expanduser().resolve()
    PROTOTYPE_CONFIG = load_dynamic_prototype_config(config_path) if config_path else load_dynamic_prototype_config()
    MODEL_ORDER = PROTOTYPE_CONFIG.get("model_names", [])
    EXPECTED_SAMPLE_INDICES = PROTOTYPE_CONFIG.get("source_sample_indices", [])
    EXPECTED_FRAME_IDS = PROTOTYPE_CONFIG.get("frame_ids", [])
    EXPECTED_COUNTS = {
...
def load_trusted_frames(path: Path, expected_frames: list[dict[str, int]]) -> dict[str, Any]:
    # The output of build_dynamic_pose_frames.py is treated as a trusted intermediate artifact. Re-
...
def build_visual_frames(
    ...
    # Join the per-frame link poses from build_dynamic_pose_frames.py with the per-visual metadata
...
def main() -> int:
    ...
    dynamic_frames_path = FRAMES_PATH
    ...
    output = {
        ...
        "source_frames_file": display_path(dynamic_frames_path),
        "source_manifest_file": display_path(DYNAMIC_MANIFEST_PATH),
