...
def load_experiment_config(config_path: Path) -> dict[str, Any]:
    ...
    if dynamic_prototype_config_value not in (None, ""):
        dynamic_prototype_config_path = resolve_project_path(
            require_non_empty_string(
                dynamic_prototype_config_value,
                "experiment_config.dynamic_prototype_config",
            )
        )
...
def iter_pose_blocks(path: Path):
    ...
    # source sample indices have the same meaning as those emitted by
    # build_dynamic_pose_frames.py.
    if not path.exists():
