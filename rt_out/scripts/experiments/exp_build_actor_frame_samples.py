...
"""Build experiment-local actor frame samples compatible with script 42.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build experiment-local actor frame samples compatible with dynamic_actor/42."
    )
    parser.add_argument(
        "--config",
        type=Path,
...
def load_experiment_config(path: Path) -> dict[str, Any]:
    ...
    if actor_time_policy != "uniform_over_actor_trajectory":
        raise ExperimentActorFrameSampleError(
