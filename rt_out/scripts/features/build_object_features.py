...
def load_experiment_config(path: Path) -> dict[str, Any]:
    ...
    actor_semantic_class: str | None = None
    actors_value = config.get("actors")
    if actors_value is not None:
        actors_config = require_object(actors_value, "experiment_config.actors")
        actors_enabled = bool(actors_config.get("enabled", False))
        actor_semantic_value = actors_config.get("semantic_class")
        if actor_semantic_value not in (None, ""):
            actor_semantic_class = require_non_empty_string(
                actor_semantic_value,
                "experiment_config.actors.semantic_class",
            ).lower()
    return {
...
def load_rt_label_rows(path: Path) -> list[dict[str, Any]]:
    try:
        ...
        raise ObjectFeatureBuildError(
            ...
            f"`python3 rt_out/scripts/rt/build_rt_labels.py --config "
...
def build_frame_objects(
    ...
    *,
    ...
    for entry_index, entry_value in enumerate(entries):
        ...
        if source == "actor":
            actor_name = str(entry.get("actor_name") or "").strip() or "actor"
            ...
            semantic = actor_semantic_class
            if semantic is None and "person" in allowed_semantics:
                semantic = "person"
            elif semantic is None and "actor" in allowed_semantics:
                semantic = "actor"
            objects.append(
                {
                    "source": "actor",
                    ...
                    "semantic_class": semantic,
                    "is_dynamic": True,
                    "dynamic_model": actor_name or "actor",
                    "centroid": centroid,
                    "volume": volume,
...
def main() -> int:
    ...
    actor_objects_extracted = 0
    frames_with_actor_objects = 0
    for frame_id in frame_ids_needed:
        ...
        frame_objects[frame_id] = build_frame_objects(
            ...
            actor_semantic_class=config["actor_semantic_class"],
        )
        actor_count = sum(1 for obj in frame_objects[frame_id] if obj.get("source") == "actor")
        actor_objects_extracted += actor_count
        if actor_count > 0:
            frames_with_actor_objects += 1

    if config["actors_enabled"] and actor_objects_extracted == 0:
        raise ObjectFeatureBuildError(
            "experiment_config.actors.enabled is true, but no actor objects were extracted from any frame"
        )
    ...
    human_skin_material_rows = 0
    if "human_skin" in config["materials_of_interest"] and feature_columns is not None:
        human_skin_column = "mat_human_skin_count"
        for row in enriched_rows:
            if parse_float(row.get(human_skin_column)) and float(row[human_skin_column]) > 0.0:
                human_skin_material_rows += 1
    ...
    print(f"actor_objects_extracted={actor_objects_extracted}")
    print(f"frames_with_actor_objects={frames_with_actor_objects}")
    print(f"human_skin_material_rows={human_skin_material_rows}")
