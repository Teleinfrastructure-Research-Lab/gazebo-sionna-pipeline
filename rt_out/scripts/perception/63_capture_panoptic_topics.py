#!/usr/bin/env python3
"""Capture Gazebo panoptic topics into experiment-local image files.

Gazebo's built-in <save> path is not emitting files on this setup, so this
wrapper subscribes to the panoptic topics through a small Gazebo Transport
utility and saves labels_map / colored_map frames manually.
"""
...
VALID_MODES = ("panoptic",)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Subscribe to Gazebo panoptic topics and save labels_map / colored_map frames."
    )
    ...
    parser.add_argument(
        ...
        choices=VALID_MODES,
        default="panoptic",
        help="Capture panoptic topics from the active Gazebo-native world.",
    )
