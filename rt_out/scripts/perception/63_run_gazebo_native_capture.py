...
The active dataset path is panoptic topic capture. This helper can launch the
panoptic world directly, and can still launch legacy split semantic / instance
worlds only when explicitly requested for debugging.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Gazebo Sim against the panoptic primary world or legacy split debug worlds."
    )
    ...
    parser.add_argument(
        ...
        choices=VALID_MODES,
        default="panoptic",
        help="Which native world variant to run. 'panoptic' is primary; 'semantic' and 'instance' require --allow-debug-split-mode.",
    )
    parser.add_argument(
        "--allow-debug-split-mode",
        action="store_true",
        help="Allow legacy semantic/instance split-world launches for debugging.",
    )
...
def main() -> None:
    ...
    world_paths = {
        ...
        "semantic": semantic_world_path,
        "instance": instance_world_path,
    }
    ...
    stdout_log = logs_root / f"{args.mode}.stdout.log"
    stderr_log = logs_root / f"{args.mode}.stderr.log"
    motion_stdout_log = logs_root / f"{args.mode}.motion.stdout.log"
    motion_stderr_log = logs_root / f"{args.mode}.motion.stderr.log"
    overwrite_targets.extend([output_dir, stdout_log, stderr_log, motion_stdout_log, motion_stderr_log])
