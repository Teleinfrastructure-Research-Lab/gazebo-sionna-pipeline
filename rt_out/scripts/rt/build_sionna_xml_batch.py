...
"""Batch wrapper that builds many per-frame Sionna XML files via build_frame_sionna_xml.py.
...
from runtime_config import PROJECT_ROOT, SCRIPT_BUILD_FRAME_SIONNA_XML  # noqa: E402

XML_SCRIPT = SCRIPT_BUILD_FRAME_SIONNA_XML
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional debug limit on the number of composed-manifest rows to process.",
    )
...
def load_composed_manifest_index(path: Path) -> list[dict[str, Any]]:
    ...
    if not rows:
        raise BatchBuildSionnaXmlError(f"composed_manifest_index.csv is empty: {path}")
...
def main() -> int:
    ...
    if args.max_frames is not None:
        if args.max_frames <= 0:
            raise BatchBuildSionnaXmlError("--max-frames must be a positive integer")
        records = records[: args.max_frames]
    elif len(records) != experiment["num_frames"]:
        raise BatchBuildSionnaXmlError(
            f"composed_manifest_index.csv has {len(records)} rows, expected {experiment['num_frames']}"
        )
