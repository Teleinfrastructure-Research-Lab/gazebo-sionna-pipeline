...
def parse_num_paths(output: str) -> int | None:
    ...
    # a single `paths_found : N` line from run_sionna_rt_sanity.py.
...
def extract_tau_stats(
    *,
    ...
    # inspect the Sionna Paths object directly without modifying the static RT sanity runner.
    result = run_command(
        [
            str(rt_python),
            "-c",
...
def load_xml_index(path: Path) -> list[dict[str, Any]]:
    ...
    if not rows:
        raise ExperimentRtBatchError(f"sionna_xml_index.csv is empty: {path}")
...
def main() -> int:
    ...
    try:
        ...
        raise ExperimentRtBatchError(str(exc)) from exc
    ...
    elif len(frame_rows) != experiment["num_frames"]:
        raise ExperimentRtBatchError(
            f"sionna_xml_index.csv has {len(frame_rows)} rows, expected {experiment['num_frames']}"
        )
