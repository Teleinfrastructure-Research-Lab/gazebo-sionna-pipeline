...
def require_blender():
    ...
    if bpy is None:
        try:
            ...
            raise SystemExit(
                "This helper requires Blender's Python runtime; use `blender --background --python ...`"
            ) from exc
        bpy = blender_api
...
def main():
    ...
    if "--help" in argv or "-h" in argv:
        print("Usage: blender --background --python convert_mesh_to_ply_blender.py -- input_mesh output.ply")
        return
    if "--" not in argv:
        raise SystemExit("Използване: blender --background --python convert_mesh_to_ply_blender.py -- input_mesh output.ply")
