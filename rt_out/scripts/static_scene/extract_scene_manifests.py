...
def main() -> int:
    ...
    parser.add_argument("--root", type=Path, default=PROJECT_ROOT,
                        help="Repository root containing myworld_rt.sdf and models/.")
    parser.add_argument("--sdf", type=Path, default=None,
                        help="Explicit SDF world path.")
    parser.add_argument("--models-root", type=Path, default=None,
                        help="Explicit model asset root.")
    parser.add_argument("--experiment-root", type=Path, default=None,
                        help="Run root used to resolve config/ and manifests/.")
    parser.add_argument("--dynamic-prototype-config", type=Path, default=None,
                        help="Dynamic prototype config loaded after CLI parsing.")
    parser.add_argument("--output-dir", type=Path, default=None,
                        help="Output directory for static_manifest.json and dynamic_manifest.json.")
    args = parser.parse_args()

    global models_dir, dynamic_models
    root_dir = args.root.expanduser().resolve()
    models_dir = (args.models_root or (root_dir / "models")).expanduser().resolve()
    ...
    prototype_config = load_dynamic_prototype_config(config_path)
    dynamic_models = set(prototype_config["model_names"])
    sdf_path = (args.sdf or (root_dir / "myworld_rt.sdf")).expanduser().resolve()
    out_dir = (args.output_dir or (
        experiment_root / "manifests" if experiment_root else
        root_dir / "rt_out/experiments/factory_panda_ur5/legacy_run_20260522_133045/manifests"
    )).expanduser().resolve()
    ...
    for model in world.findall("model"):
        ...
        (dynamic_manifest if model_entry["model"] in dynamic_models else static_manifest).append(model_entry)
