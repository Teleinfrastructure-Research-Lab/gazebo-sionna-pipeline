...
def require_blender() -> None:
    ...
    try:
        ...
        from mathutils import Matrix as blender_matrix
...
REFLECTIVE_TABLE_MESH_RELATIVE = Path(
    "rt_out/experiments/factory_panda_ur5/legacy_run_20260522_133045/static_scene/converted_meshes/models/furniture/"
...
PICKING_SHELVES_CONVERTED_MESH_RELATIVE = Path(
    "rt_out/experiments/factory_panda_ur5/legacy_run_20260522_133045/static_scene/converted_meshes/models/furniture/picking_shelves/base_visual.ply"
)
...
def parse_job_args() -> tuple[Path, Path]:
    ...
    if "--" not in argv:
        raise SystemExit(
            "Usage: blender --background --python static_scene_blender_merge_worker.py -- job.json result.json"
        )
...
def main() -> None:
    if "--help" in sys.argv or "-h" in sys.argv:
        print("Usage: blender --background --python static_scene_blender_merge_worker.py -- job.json result.json")
        return
    require_blender()
    ...
    root = (
        Path(str(payload.get("root", Path(__file__).resolve().parents[3])))
        .expanduser()
        .resolve()
    )
